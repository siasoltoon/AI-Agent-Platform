...
class AgentExecutor:
    ...
    _TERMINAL_ALIASES = {"type", "cat", "dir", "ls", "pwd", "where", "findstr", "fc", "tree", "more", "echo", "mkdir", "mktemp", "whoami", "hostname", "ver", "date", "time", "python", "py", "pytest", "pip", "pip3", "git", "uvicorn", "ruff", "black", "mypy", "node", "npm", "npm.cmd", "npx", "vite", "yarn", "pnpm", "dotnet", "java", "javac", "go", "cargo", "rustc", "pytest.exe", "python.exe", "node.exe", "npm.exe", "git.exe"}
    ...
    def _alias_command(self, name: str, args: dict[str, Any]) -> str:
        ...
        if name in {"type", "cat"}: return f"type {quoted}".strip()
        if name in {"dir", "ls", "pwd", "where", "tree", "more"}: return f"{name} {quoted}".strip()
        if name == "findstr":
    ...
    def _tool(self, name: str, args: dict[str, Any]) -> Any:
        ...
        if name == "write_file":
            path = self._safe_path(self._arg(args, "path", "file_path", "filepath")); content = str(self._arg(args, "content", "text", "body")); result = self.write_file.execute(str(path), content); return {**result, "path": self._relative(path), "content": content}
        if name == "file_exists":
        ...
        if name == "list_directory":
            path = self._safe_path(self._arg(args, "path", "directory", default=".")); result = self.list_directory.execute(str(path)); return {"ok": True, **result, "path": self._relative(path)}
    ...
    def _verify_evidence(self, task: str, records: list[dict[str, Any]], policy: MissionPolicy | None = None) -> dict[str, Any]:
        policy = policy or MissionPolicy.from_task(task); successful = [r for r in records if r.get("ok") is True]; failed = [r for r in records if r.get("ok") is not True]; checks: list[dict[str, Any]] = []
        ...
        for record in writes:
            ...
            if not rel: continue
            path = self._safe_path(rel); exists = path.is_file(); checks.append({"type": "file_exists", "path": rel, "passed": exists})
        ...
        for record in reads:
            ...
            if rel in write_contents and isinstance(write_contents[rel], str): checks.append({"type": "read_content_matches_write", "path": rel, "expected_content": write_contents[rel], "actual_content": result.get("content"), "passed": result.get("content") == write_contents[rel]})
        for record in successful:
            ...
            if tool == "make_directory":
                ...
                if src and dst: checks.extend([{"type": "move_destination_exists", "path": dst, "passed": self._safe_path(dst).exists()}, {"type": "move_source_absent", "path": src, "passed": not self._safe_path(src).exists()}])
        ...
        for record in successful:
            if record.get("tool") in {"terminal", *self._TERMINAL_ALIASES}:
                result = record.get("result", {}); checks.append({"type": "terminal_success", "command": result.get("command"), "passed": result.get("code") == 0 and not result.get("timed_out", False)})
        required = [c for c in checks if c["type"] != "terminal_success"]; failed_checks = [c for c in required if not c.get("passed")]; mutating = any(r.get("tool") in self._MUTATING_TOOLS for r in successful); policy_evidence = policy.evidence(records); verified = bool(successful) and not failed_checks and policy_evidence["compliant"]
        if writes and self._verification_requested(task) and not reads: verified = False
    ...
    def _result_from_records(actions: list[dict[str, Any]], records: list[dict[str, Any]], evidence: dict[str, Any], summary: str = "Task completed.") -> dict[str, Any]:
        return {"status": "completed", "execution_mode": "agentic", "summary": summary, "steps": actions, "execution_evidence": evidence, "tool_records": records}
    ...
    def _verified_result(self, task: str, actions: list[dict[str, Any]], records: list[dict[str, Any]], summary: str = "Task completed.", policy: MissionPolicy | None = None) -> dict[str, Any] | None:
        evidence = self._verify_evidence(task, records, policy); return self._result_from_records(actions, records, evidence, summary) if evidence["verified"] else None
    ...
    def _partial_result(self, task: str, actions: list[dict[str, Any]], records: list[dict[str, Any]], policy: MissionPolicy | None = None) -> dict[str, Any] | None:
        return None if not records else self._result_from_records(actions, records, self._verify_evidence(task, records, policy))
...
MISSION POLICY: If this mission is read-only, you MUST NOT mutate files, directories, or the workspace. Do not use write_file, make_directory, copy_file, move_file, delete_file, or terminal. Use only observation tools. A policy violation is a failed mission and can never be marked verified. These restrictions remain authoritative during recovery."""
        if verification_requested: system += """
...
The execution runtime independently validates the real filesystem and generated evidence; never fabricate verification results."""
        conversation = f"{system}\n\nTASK:\n{task.strip()}\n\nWORKSPACE:\n{self.workspace}"; actions: list[dict[str, Any]] = []; records: list[dict[str, Any]] = []
        for step in range(1, self.max_steps + 1):
            try: response = self.ollama.generate(conversation, timeout=self.ollama.timeout)
                ...
                if not records: raise AgentExecutionError("Agent stopped without executing any tool action.", partial_result=partial) from exc
            ...
            try: decision = self._normalize_decision(ActionParser.extract_object(response.get("response", "")))
                ...
                actions.append({"step": step, "decision_error": str(exc)}); conversation += "\n\nACTION FORMAT ERROR: Your previous response could not be parsed as a JSON object. Recover by returning ONLY one valid JSON object. No markdown, prose, or code fence. Use exactly the required action schema and continue the task."; continue
            actions.append({"step": step, "decision": decision})
            if decision.get("action") == "done":
                if not records: conversation += "\n\nPREMATURE COMPLETION REJECTED: You have executed zero tool actions. The task is not complete. Do not return done again. Your next response MUST be a tool action that advances the original task, such as write_file/read_file/file_exists/terminal as appropriate."; continue
            ...
            if not policy.allows_tool(tool, args):
                violation = {"step": step, "tool": tool, "ok": False, "policy_violation": True, "error_type": "MissionPolicyViolation", "error": policy.describe_violation(tool, args)}; records.append(violation); conversation += f"\n\nPOLICY VIOLATION step {step}: {violation['error']} Do not retry the prohibited action. Continue only with tools allowed by the mission policy."; continue
            ...
            records.append(record); serialized = json.dumps(result, ensure_ascii=False, default=str); serialized = serialized[: self.max_output_chars] + "...<truncated>" if len(serialized) > self.max_output_chars else serialized; conversation += f"\n\nOBSERVATION step {step}: tool={tool}\n{serialized}\n\nContinue, recover, or verify before done."
            if verification_requested and tool == "write_file" and record.get("ok") is True: conversation += f"\n\nMANDATORY NEXT ACTION: The write to {record.get('result', {}).get('path')!r} succeeded. You MUST now call read_file for exactly that path and compare its returned content character-for-character with the requested content before any done action."
        verified = self._verified_result(task, actions, records, policy=policy)
        if verified: return verified
