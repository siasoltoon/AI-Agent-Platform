...
def _safe_agent_status() -> dict[str, Any]:
    try:
        return agents.runtime.health_check()
...
def dashboard_summary() -> dict[str, Any]:
    ...
    agent = _safe_agent_status()
    worker_data = _safe_workers()
    return {
        "generated_at": time.time(),
        "system": {
            "live": live,
            "ready": {"status": "ready" if ready else "not_ready", "error": ready_error},
        },
        "tasks": summary,
        "agent": agent,
        "workers": worker_data,
...
def dashboard_diagnostics() -> dict[str, Any]:
    ...
    try:
        tasks.TASK_STORE.ping()
    except Exception as exc:
        checks.append({"name": "Task Store", "status": "fail", "detail": str(exc)})
    else:
        checks.append({"name": "Task Store", "status": "pass", "detail": "SQLite reachable"})

    try:
        agent = agents.runtime.health_check()
    except Exception as exc:
        checks.append({"name": "Agent Worker", "status": "fail", "detail": str(exc)})
    else:
        checks.append({"name": "Agent Worker", "status": "pass", "detail": agent})

    worker_data = _safe_workers()
    if worker_data.get("error"):
        checks.append({"name": "Worker Registry", "status": "fail", "detail": worker_data["error"]})
    else:
        checks.append({
            "name": "Worker Registry",
            "status": "pass",
            "detail": f"{len(worker_data.get('workers', []))} registered worker(s)",
        })
