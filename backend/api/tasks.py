...
async def create_task(task: TaskRequest) -> TaskResponse:
    ...
    TASK_STORE[task_id] = {
        ...
        "result": None,
        "error": None,
        "metadata": {**task.metadata, "prompt_length": len(task.prompt), "command": command},
    }
    ...
    try:
        ...
        TASK_STORE[task_id]["metadata"].update({"execution_mode": result.get("execution_mode", "agentic")})
        nested = result.get("result")
        if isinstance(nested, dict):
            TASK_STORE[task_id]["metadata"].update({"steps": nested.get("steps", 1), "orchestration_mode": nested.get("mode", "agentic")})
        return TaskResponse(**TASK_STORE[task_id])
    except TimeoutError as exc:
        TASK_STORE[task_id]["status"] = TaskStatus.FAILED
        TASK_STORE[task_id]["completed_at"] = time.time()
        TASK_STORE[task_id]["error"] = str(exc) or "Task execution timed out."
        raise HTTPException(status_code=504, detail={"task_id": task_id, "message": "Task execution timed out.", "error": str(exc)}) from exc
    except Exception as exc:
        TASK_STORE[task_id]["status"] = TaskStatus.FAILED
        TASK_STORE[task_id]["completed_at"] = time.time()
        TASK_STORE[task_id]["error"] = str(exc)
        raise HTTPException(status_code=502, detail={"task_id": task_id, "message": "Task execution failed.", "error": str(exc)}) from exc
