(() => {
  ...
  const statusClass = (status) => ({completed:"success",running:"success",queued:"warning",failed:"danger",cancelled:"neutral"}[normalizeStatus(status)] || "neutral");
  ...
  function countTasks(){const c={total:state.tasks.length,queued:0,running:0,completed:0,failed:0,cancelled:0};state.tasks.forEach(t=>{const status=normalizeStatus(t.status);if(c[status]!==undefined)c[status]++;});return c;}
