...
class Worker:
    def __init__(self, worker_id: str, isolation_policy: WorkerIsolationPolicy | None = None):
        self.worker_id = worker_id
        self.isolation_policy = isolation_policy or WorkerIsolationPolicy(WORKER_ISOLATION_ROOT)
        self.status = "idle"
    ...
    def _release_task_id(self, task_id: str | None) -> None:
        if task_id:
            with self._idempotency_lock: self._inflight_task_ids.discard(task_id)
    ...
    def cancel(self, task_id: str) -> bool:
        task_id = str(task_id or "").strip(); received_at = time.monotonic()
        ...
        with self._cancellation_lock:
            ...
            if event is None:
                ...
                return False
            self._cancellation_requested_at[task_id] = received_at; event.set()
    ...
    def _cancel_elapsed_ms(self, task_id: str | None, now: float | None = None) -> float | None:
        if not task_id: return None
        with self._cancellation_lock: requested_at = self._cancellation_requested_at.get(task_id)
        if requested_at is None: return None
        return ((now if now is not None else time.monotonic()) - requested_at) * 1000

    def execute(self, job: dict[str, Any]) -> dict[str, Any]:
        task_id = str(job.get("task_id") or "").strip() or None
        execution_started = time.monotonic(); cached = self._claim_task_id(task_id)
        if cached is not None: return cached
        if not self._execution_lock.acquire(blocking=False):
            self._release_task_id(task_id); raise RuntimeError("Worker is busy executing another task.")
        cancellation_event = self._register_cancellation(task_id)
        self.status = "running"; self.started_at = time.time(); self.last_error = None
        logger.info("Task execution started task_id=%s", task_id)
        try:
            prompt = job.get("prompt") or job.get("task")
            if not prompt or not str(prompt).strip(): raise ValueError("Task prompt is required.")
            prompt = str(prompt).strip()
            if len(prompt) > MAX_PROMPT_CHARS: raise ValueError(f"Task prompt cannot exceed {MAX_PROMPT_CHARS} characters.")
            model = str(job.get("model") or DEFAULT_MODEL).strip()
            if not model or len(model) > MAX_MODEL_CHARS: raise ValueError(f"Model identifier must be between 1 and {MAX_MODEL_CHARS} characters.")
            timeout = int(job.get("timeout") or WORKER_TIMEOUT)
            if timeout < 1 or timeout > MAX_TIMEOUT_SECONDS: raise ValueError(f"Worker timeout must be between 1 and {MAX_TIMEOUT_SECONDS} seconds.")
            metadata = job.get("metadata") or {}
            if not isinstance(metadata, dict) or len(metadata) > MAX_METADATA_KEYS: raise ValueError(f"Task metadata cannot contain more than {MAX_METADATA_KEYS} keys.")
            workspace = self.isolation_policy.resolve_workspace(metadata.get("workspace"))
            execution_profile = str(metadata.get("execution_profile", "normal")).strip().lower()
            if execution_profile not in {"normal", "large"}: raise ValueError("execution_profile must be either 'normal' or 'large'.")
            ...
            cleanup_started = time.monotonic(); cancelled = bool(cancellation_event and cancellation_event.is_set()); cancel_to_cleanup_ms = self._cancel_elapsed_ms(task_id, cleanup_started)
            ...
            self._release_cancellation(task_id); self.status = "idle"; self.started_at = None; self._execution_lock.release()
...
def execute(request: ExecuteRequest) -> dict[str, Any]:
    try: return worker.execute(request.model_dump())
        ...
        worker.status = "idle"; logger.error("Worker execution failed: %s\n%s", exc, traceback.format_exc())
...
def cancel(task_id: str) -> dict[str, Any]:
    logger.warning("[CANCEL] HTTP endpoint entered task_id=%s", task_id); result = worker.cancel(task_id)
    if result:
        logger.warning("[CANCEL] HTTP endpoint returning cancellation_requested task_id=%s", task_id); return {"status": "cancellation_requested", "task_id": task_id}
    logger.warning("[CANCEL] HTTP endpoint returning not_inflight task_id=%s", task_id); return {"status": "not_inflight", "task_id": task_id}
